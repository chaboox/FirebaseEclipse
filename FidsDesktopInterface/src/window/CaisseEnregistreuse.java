...
public class CaisseEnregistreuse extends JPanel {
	...
	public CaisseEnregistreuse() throws IOException {
		...
		list.setBounds(33, 175, 60, 156);
		textPane = new JTextPane();

		textPane.setBounds(33, 443, 160, 19);
		textPane.setText(" À payer : " + sumPrice(listModel) + " €");
		// add(textPane);

		textPane2 = new JTextPane();
		textPane2.setEditable(false);
		textPane2.setBounds(33, 768, 500, 29);
		...
		textPane3.setBounds(33, 803, 500, 29);
		...
		textPane4.setBounds(33, 838, 500, 29);
		...
		textField.setBounds(33, 137, 243, 29);
		add(textField);
		textField.setColumns(10);

		textField_1 = new JTextField();
		textField_1.setFont(f2);
		textField_1.setBounds(280, 137, 214, 29);
		add(textField_1);
		textField_1.setColumns(10);

		textField_2 = new JTextField();
		textField_2.setFont(f2);
		textField_2.setBounds(497, 137, 214, 29);
		add(textField_2);
		textField_2.setColumns(10);

		JTextPane textFieldP = new JTextPane();
		textFieldP.setEditable(false);
		textFieldP.setBounds(33, 107, 143, 29);
		...
		textField_1P.setBounds(280, 107, 114, 29);
		...
		textField_2P.setBounds(497, 107, 114, 29);
		...
		ajouter.setBounds(717, 137, 217, 29);
		ajouter.setFont(f2);
		add(ajouter);

		scrollPane = new JScrollPane(list);
		scrollPane.setBounds(33, 175, 160, 156);
		// add(scrollPane);
		String title[] = { "Produit", "Nombre", "H.T", "T.T.C" };

		table = new JTable();

		table.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "Produit", "Nombre", "H.T", "T.T.C" }));
		tableModel = (DefaultTableModel) table.getModel();
		table.setRowHeight(30);
		table.setFont(f2);
		table.getTableHeader().setFont(f);
		// tableModel.addRow(title);
		table.setBounds(205, 175, 221, 256);
		JScrollPane sp = new JScrollPane(table);
		sp.setBounds(33, 175, 900, 556);
		add(sp);
			...
			public void onChildAdded(DataSnapshot arg0, String arg1) {
				...
				for (DataSnapshot child : arg0.getChildren()) {
					if (arg0.getKey().toString().equals("-1")) {
						...
						textPane3.setText(
								"Reste à payer : " + df2.format(sumPrice(tableModel, 3) - totalFidz * 0.5) + " €");
						textPane4.setText(
								"Fidz gagnés : " + (df2.format((sumPrice(tableModel, 3) - totalFidz * 0.5) / 10)));
						if ((sumPrice(tableModel, 3) - totalFidz * 0.5) < 0)
							textPane4.setText("Fidz gagnés : 0.0");
						arg0.getRef().removeValueAsync();
					} else {
						System.out.println(child.getValue().toString());
						MainFram.firebaseFunction.getProduitRef().child(child.getValue().toString())
								.addListenerForSingleValueEvent(new ValueEventListener() {
									...
									public void onDataChange(DataSnapshot arg1) {
										...
										for (int i = 0; i < tableModel.getRowCount(); i++)
											if (arg1.child("nom").getValue().toString()
													.equals(tableModel.getValueAt(i, 0)))
												f = i;
										if (f == -1)
											tableModel
													.addRow(new String[] { arg1.child("nom").getValue().toString(), "1",
															arg1.child("prix").getValue().toString() + "",
															df2.format(Double.parseDouble(
																	arg1.child("prix").getValue().toString()) * 1.2)
																	+ "" });

										else {
											tableModel.insertRow(f, new String[] {
													(String) tableModel.getValueAt(f, 0),
													"" + (Integer.parseInt(
															(String) tableModel.getValueAt(f, 1)) + 1),
													"" + df2.format(
															(Double.parseDouble((String) tableModel.getValueAt(f, 2)))
																	* (Integer.parseInt(
																			(String) tableModel.getValueAt(f, 1))
																			+ 1)
																	/ (Integer.parseInt(
																			(String) tableModel.getValueAt(f, 1)))),
													"" + df2.format(
															(Double.parseDouble((String) tableModel.getValueAt(f, 3)))
																	* (Integer.parseInt(
																			(String) tableModel.getValueAt(f, 1)) + 1)
																	/ (Integer.parseInt(
																			(String) tableModel.getValueAt(f, 1)))) });
											tableModel.removeRow(f + 1);
											// tableModel.setValueAt(Integer.parseInt((String)tableModel.getValueAt(f,
											// 1))+1, f, 1);
											// tableModel.setValueAt((Double.parseDouble((String)tableModel.getValueAt(f,
											// 1))+1)* (Double.parseDouble((String)tableModel.getValueAt(f, 2))), f, 2);
											// tableModel.setValueAt(0, f, 2);
											// tableModel.setValueAt(2*
											// (Double.parseDouble((String)tableModel.getValueAt(f, 2))), f, 2);
											// tableModel.setValueAt(2*
											// (Double.parseDouble((String)tableModel.getValueAt(f, 3))), f, 3);

											System.out.println("2  ="
													+ (Double.parseDouble((String) tableModel.getValueAt(f, 1)) + 1));
										}
										double sumPay = sumPrice(tableModel, 3);
										textPane3.setText(
												"Reste à payer : " + df2.format(sumPay - totalFidz * 0.5) + " €");
										textPane4.setText(
												"Fidz gagnés : " + (df2.format((sumPay - totalFidz * 0.5) / 10)));
										...
										System.out.println(
												"cpt + " + Integer.parseInt((String) tableModel.getValueAt(f, 1)));
		...
		ajouter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!textField.getText().equals("") && !textField_1.getText().equals("")
					...
					tableModel.addRow(new String[] { textField.getText(), textField_2.getText(),
							Double.parseDouble(textField_1.getText()) + "",
							df2.format(Double.parseDouble(textField_1.getText()) * 1.2) + "" });
					textField.setText("");
					textField_1.setText("");
					textField_2.setText("");
					textPane.setText(" À payer : " + sumPrice(listModel) + " €");
